...
<div class="bg-white p-4 rounded-lg shadow">
    ...
    <div class="grid grid-cols-7 gap-1 mb-4" wire:key="calendario-{{ $fechaActual }}-{{ time() }}">
        ...
        @for ($day = 1; $day <= $daysInMonth; $day++)
            ...
            <div wire:click="seleccionarFecha('{{ $currentDate }}')"
                ...
                <form wire:submit.prevent="guardarPractica">
                    ...
                    <div class="mb-4" x-data="{ mostrarCalculadora: false }">
                        ...
                        <div class="flex gap-2">
                            <input type="number" step="0.5" min="0.5" max="12" wire:model="horas"
                                class="w-full px-3 py-2 border rounded" placeholder="Ej: 4.5"
                                x-bind:readonly="mostrarCalculadora" wire:key="horas-input-{{ now()->timestamp }}">
                            <button type="button" @click="mostrarCalculadora = !mostrarCalculadora"
                        ...
                        <div x-show="mostrarCalculadora" x-cloak class="mt-2 p-4 border rounded-lg bg-gray-50">
                            <div class="grid grid-cols-2 gap-4 mb-4">
                                <div>
                                    ...
                                    <input type="time" wire:model.live="hora_inicio" wire:change="calcularHoras"
                                ...
                                <div>
                                    ...
                                    <input type="time" wire:model.live="hora_fin" wire:change="calcularHoras"
                                        class="w-full px-3 py-2 border rounded text-sm">
                                    @error('hora_fin')
                                        <span class="text-red-500 text-xs">{{ $message }}</span>
                                    @enderror
                            ...
                            <div class="flex justify-end gap-2">
                                <button type="button" @click="mostrarCalculadora = false"
                                    class="px-3 py-1 text-sm text-gray-600 hover:bg-gray-100 rounded">
                                    Cancelar
                                </button>
                                <button type="button" wire:click="calcularHoras" @click="mostrarCalculadora = false"
                ...
                <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                    <div class="bg-white rounded-lg p-6 w-full max-w-sm">
                        ...
                        <p class="text-sm text-gray-600 mb-6">
                            ¿Estás seguro de que deseas eliminar la práctica del día
                            {{ \Carbon\Carbon::parse($fechaSeleccionada)->format('d/m/Y') }}? Esta acción no se
                            puede deshacer.
                        </p>
                        <div class="flex justify-end gap-3">
                            ...
                            <button wire:click="eliminarPractica"
